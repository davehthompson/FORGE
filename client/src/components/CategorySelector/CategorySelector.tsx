        ...
        <CardContent>
          ...
          <div className="space-y-3 mb-8">
            {company.spendingCategories.map((category, index) => {
              const isSelected = selectedSpendingCategory === category;
          ...
          {selectedSpendingCategory && (
            <div className="mb-6 p-4 bg-ramp-solar/20 rounded-lg border border-ramp-solar">
              <p className="text-sm text-ramp-slate">
                <strong>Selected:</strong> {selectedSpendingCategory}
              </p>
              <p className="text-xs text-ramp-sage mt-1">
                All generated invoices, receipts, quotes, and contracts will use {currentCurrency.code} ({currentCurrency.symbol}) and be related to this category.
              </p>
            </div>
          )}
